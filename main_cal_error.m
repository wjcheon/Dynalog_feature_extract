...
title(sprintf('Velocity error of MLC (Taylor method)\n: %s\n', strrep(BankAFileName,'_',' ')))
view([90 270])
...
view([90 270])
%
figure(10), subplot(2,1,2), plot(ExpectedPosition_Velocity_Taylor_single), title('Velocity (Taylor)')
xlabel('number of dynalog'), ylabel('~'), grid on
%%

number_of_conduct = 2000;
number_of_element = 50;
PositionError_single = PositionError(:,30);
PositionError_clt = clt_wjcheon(PositionError_single , number_of_conduct, number_of_element);
figure(20), subplot(1,2,1), histogram(PositionError_single), title('PositionError'), grid on;
subplot(1,2,2), histogram(PositionError_clt), title('PositionError (CLT)'), grid on, xlim([-0.4 0.4]);
...
VelocityError_clt = clt_wjcheon(VelocityError_single , number_of_conduct, number_of_element);
figure(20), subplot(1,2,1), histogram(VelocityError_single), title('VelocityError'), grid on;
subplot(1,2,2), histogram(VelocityError_clt), title('VelocityError (CLT)'), grid on;
