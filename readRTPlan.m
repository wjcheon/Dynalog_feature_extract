function [ output_args, RTplanFileName, RTplanPathName] = readRTPlan()
...
for iter1 = 1: numberOfBeamSequence
    ...
    eval(sprintf('RTPlanAnalyzed.Item_%d.MLCPosition_bankA = MLCPosition(:,61:end);',iter1))
    eval(sprintf('RTPlanAnalyzed.Item_%d.MLCPosition_bankB = MLCPosition(:,1:60);',iter1))
    %
    
end
%     a=0;
    if isfield(dicomInformation,'InstanceCreationDate')
        RTPlanAnalyzed.InstanceCreationDate = getfield(dicomInformation,'InstanceCreationDate');
    end
    %
    if isfield(dicomInformation,'StudyDate')
        RTPlanAnalyzed.StudyDate = getfield(dicomInformation,'StudyDate');
    end
    %
    if isfield(dicomInformation,'PatientID')
        RTPlanAnalyzed.PatientID = getfield(dicomInformation,'PatientID');
    end
    %
     if isfield(dicomInformation,'PatientName')
        RTPlanAnalyzed.PatientName = getfield(dicomInformation,'PatientName');
     end
    eval(sprintf('RTPlanAnalyzed.dicomInformation = dicomInformation;'))
    output_args = RTPlanAnalyzed;
end
