function [ output_args ] = fDrawMLC( bankA_pos, bankB_pos, index_, handles)
...
a=0;
% %%`
% [filename, filepath] = uigetfile('*.mac','Open the SpecCalc Spectrum');
% item_list = dir(fullfile(filepath, '*.mac'))
% fid = fopen(fullfile(filepath,item_list(1).name),'r');
...
DynamicLeafData_A_bank = transpose(bankA_pos(index_,:).*0.1);
...
DynamicLeafData_B_bank = transpose(bankB_pos(index_,:).*0.1);
